...
librewolf-xorigin-ref-description = 仅在同域时发送引用来源。
librewolf-xorigin-ref-warning1 = 这可能会导致故障。此外，即使发送的引用也会被修剪。
librewolf-webgl-description = WebGL 是强大的指纹识别向量。
librewolf-webgl-warning1 = 如果需要启用，请考虑使用 Canvas Blocker 等扩展。
librewolf-rfp-description = 防指纹追踪是同类中最好的反指纹工具。
librewolf-rfp-warning1 = 如果需要禁用，请考虑使用 Canvas Blocker 等扩展。
librewolf-letterboxing-description = Letterboxing 在窗口周围应用边距，返回有限的一组标准分辨率。
